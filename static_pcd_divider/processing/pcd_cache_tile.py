...
def read_data_from_csv(Ri,filename):
    ...
    with open(filename, 'r') as file:
        ...
        for row in reader:
            ...
            if data['id'] not in Ri.cached_data or int(float(row['Time'])) > float(Ri.cached_data[data['id']]['valid_till']) or int(float(row['Valid_Till'])) < float(Ri.cached_data[data['id']]['valid_till']):
                cache_miss+=1
...
# Example usage
# Ri = RSU(120)  # Initialize RSU with remaining cache size of 100
...
# filename = '64_validity_with_size.csv'  # Replace with the actual filename/path
...
with open('hits_32.txt', 'a') as f:
    f.write(' '.join(map(str, hits)) + '\n')

# Write misses to a file
with open('misses_32.txt', 'a') as f:
